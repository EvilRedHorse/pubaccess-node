...
// TestSpdConfigPersistCompat confirms that the compat code for the writebps
// json tag update performs as expected
func TestSpdConfigPersistCompat(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
...
func checkWriteBPS(sc *SpdConfig, writeBPS int64) error {
