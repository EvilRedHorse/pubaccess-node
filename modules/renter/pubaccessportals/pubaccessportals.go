...
type (
	// SkynetPortals manages a list of known public access portals by persisting the
	// list to disk.
	...
	// persistEntry contains a public access portal and whether it should be listed as
...
func New(persistDir string) (*SkynetPortals, error) {
	...
	if err != nil {
		return nil, errors.AddContext(err, fmt.Sprintf("unable to initialize the pubaccess portal list persistence at '%v'", aop.FilePath()))
	}
...
func (sp *SkynetPortals) UpdatePortals(additions []modules.SkynetPortal, removals []modules.NetAddress) error {
	...
	if err != nil {
		return errors.AddContext(err, fmt.Sprintf("unable to update pubaccess portal list persistence at '%v'", sp.staticAop.FilePath()))
	}
...
// validatePortalChanges validates the changes to be made to the public access portals
// list.
