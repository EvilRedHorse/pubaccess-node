...
import (
	...
	"github.com/EvilRedHorse/pubaccess-node/modules/renter/hostdb/hosttree"
...
func (hdb *HostDB) CheckForIPViolations(hosts []types.SiaPublicKey) ([]types.SiaPublicKey, error) {
	if hdb.staticHostTree.IPRestriction() == 0 {
		return nil, nil
	}
	...
	filter := hosttree.NewFilter(hdb.staticDeps.Resolver(), hdb.staticHostTree.IPRestriction())
...
// IPRestriction returns the number of allowed hosts from the same IP address
...
// SetIPRestriction the number of allowed hosts from the same IP address
